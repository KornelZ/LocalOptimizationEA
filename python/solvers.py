...
def generate_population(chunk, dim):
    return np.random.rand(chunk, dim)

def mutate(X, chunk, dim):
    return X + np.random.normal(chunk, dim)
...
def random_search(fun, lbounds, ubounds, budget):
    ...
    while budget > 0:
        ...
        if X is None:
            X = generate_population(chunk, dim)
        X = mutate(X, chunk, dim)
        X = lbounds + (ubounds - lbounds) * X
